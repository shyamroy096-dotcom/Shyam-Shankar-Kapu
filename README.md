## Hi, I'm Shyam Shankar Kapu 👋  
### Java Full Stack Developer  

🚀 **Skills**
- Java, Python, C (Basics)  
- Spring Boot, REST APIs, JDBC  
- MySQL, SQL, PL/SQL, NoSQL  
- HTML, CSS, JavaScript, React  
- Git, VS Code, Eclipse  
- OOP, CRUD, Backend Development  


💻 **Projects**
- Student Management System (Spring Boot + MySQL)  
- Task Tracker CLI (Java + JDBC + MySQL)  

📫 **Reach Me**
- Email: shyamroy096@gmail.com  
- LinkedIn: www.linkedin.com/in/kapu-shyam-shankar-57949627b/  
- GitHub: github.com/shyamroy096-dotcom  

🔥 I love building full-stack applications and improving my backend development skills.  
Always excited to learn new technologies and work on meaningful projects.
